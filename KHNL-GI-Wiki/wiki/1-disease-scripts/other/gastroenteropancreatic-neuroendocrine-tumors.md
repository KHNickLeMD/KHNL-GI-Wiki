---
title: "Gastroenteropancreatic Neuroendocrine Tumors (GEP-NETs)"
...
```mermaid
flowchart TD
    A["GEP-NET<br/>(histology + Ki-67 grade; SSTR-PET; CT/MRI)"] --> G{"Differentiation / grade?"}
    G -->|"Poorly differentiated NEC"| NEC["Platinum-based chemotherapy<br/>(separate aggressive pathway)"]
    G -->|"Well-differentiated NET (G1–G3)"| L{"Localized vs metastatic?"}
    L -->|"Localized, small, low-grade luminal"| ER["Endoscopic resection<br/>(gastric type 1, small rectal/duodenal)"]
    L -->|"Localized, larger / deeper /<br/>node+ / type 3 gastric / appendiceal >2 cm"| SX["Surgical resection"]
    L -->|"Metastatic / unresectable"| F{"SSTR-positive?"}
    F -->|"Yes"| SSA["Somatostatin analog (octreotide/lanreotide)<br/>→ on progression: PRRT (Lu-177 dotatate),<br/>everolimus, sunitinib (PanNET), liver-directed therapy"]
    F -->|"No"| NEG["SSTR-negative pathway:<br/>everolimus, sunitinib (PanNET),<br/>chemotherapy (e.g., CAPTEM)"]
    A -.->|"Functioning tumor"| SYM["Hormone-syndrome control<br/>(SSA ± telotristat for carcinoid;<br/>PPI for gastrinoma)"]
```
