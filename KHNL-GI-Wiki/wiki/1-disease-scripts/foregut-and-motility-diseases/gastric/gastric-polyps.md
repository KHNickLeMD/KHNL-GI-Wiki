...
- **H. pylori status** in every case: biopsy-based when sampling is available; otherwise **serology, urea breath test, or stool antigen** ([[test-and-treat|test and treat]]).
...
```mermaid
flowchart TD
    A["New gastric polyp(s) on HD-WLE + IEE<br/>adequate cleaning, distension, photo-documentation"] --> B{"Solitary or multiple?"}
    B -->|Solitary| C["Resect the polyp<br/>+ systematic mucosal biopsies<br/>(updated Sydney system)"]
    B -->|Multiple| D{"Are all polyps<br/>believed to be FGPs?"}
    D -->|Yes| E["Resect largest suspected FGP<br/>Resect any FGP >10 mm or visually atypical"]
    D -->|No| F["Resect largest suspected FGP (>10 mm or atypical too)<br/>Resect all non-FGPs if expertise/time allow<br/>At minimum resect the largest non-FGP<br/>Separate jar per suspected polyp type"]
    F --> G["Systematic mucosal biopsies<br/>(updated Sydney system / MAPS II)"]
    E --> G
    C --> H["Manage and survey per histology<br/>of polyp + adjacent mucosa"]
    G --> H
```
