---
title: "Serrated Polyposis Syndrome"
category: disease-script
tags: [serrated-polyposis, sps, hyperplastic-polyposis, sessile-serrated, colorectal-cancer, polyposis, mutyh]
created: 2026-05-19
updated: 2026-08-26
sources: [acg-2015-hereditary-gi-cancer]
---

## Assessment

### Establishing the Diagnosis

- Predisposition to serrated polyps throughout the colorectum + increased risk of [[colorectal-cancer|CRC]] [[acg-2015-hereditary-gi-cancer]]
- Previously called hyperplastic polyposis syndrome
- No clear genetic etiology established

**WHO diagnostic criteria (any one must be met)** — the "updated WHO" criteria as reproduced in [[acg-2015-hereditary-gi-cancer]]:

- (i) ≥5 serrated polyps proximal to the sigmoid colon, with ≥2 of these being >10 mm in diameter; OR
- (ii) Any number of serrated polyps proximal to the sigmoid colon in an individual who has a **first-degree relative (FDR) with SPS**; OR
- (iii) >20 serrated polyps of any size, distributed throughout the large intestine

> **Gap:** WHO revised the SPS criteria in 2019 (rectal polyps counted, thresholds changed). No ingested source carries the revision — the criteria above are the pre-2019 version as printed in ACG 2015. A current WHO/ESGE polyposis source would be needed.

**Polyp spectrum**:

- Original criteria included hyperplastic polyps only
- Preferred term now "serrated polyposis syndrome" — reflects inclusion of sessile serrated adenomas/polyps (SSA/Ps) and traditional serrated adenomas (TSAs) alongside hyperplastic polyps

**Epidemiology**:

- True prevalence unknown; previously estimated at 1:100,000 (28 of 50,148 screening-colonoscopy participants, 0.06%)
- Higher in screening-detected cohorts: **5 of 755 (0.66%) — i.e. 1 in 151** — gFOBT-positive patients met SPS criteria 1 and/or 3 at index [[colonoscopy]] in the NHS Bowel Cancer Screening Programme; a Barcelona study found **8 of 2,355 (0.34%)** in [[colorectal-cancer-screening|fecal immunochemical test]]-positive patients
- Strong association with cigarette smoking (current smoking 47% vs 17% colonoscopy controls / 12% population controls); smoking is not thought to cause SPS but may intensify phenotypic expression
- Mean age at diagnosis ~48 years; pancolonic distribution (89–96% of index cases)

**Genetics**:

- Clear genetic etiology has not been defined for SPS; routine genetic testing is NOT currently recommended
- Overlap with [[mutyh-associated-polyposis|MAP]] runs in one direction more than the other: **3 of 17 biallelic MUTYH carriers (18%) met SPS criteria**, whereas only **1 of 126 SPS patients (0.8%)** carried biallelic MUTYH. In both series the patients meeting SPS criteria also had a history of adenomas — hence testing MUTYH only when adenomas are concurrent and/or there is a family history of adenomas
- GREM1 duplication has been described in hereditary mixed polyposis syndrome (HMPS), which may overlap with SPS
- Conventional adenomas are found in the majority of SPS patients (78–80%). In CRCs arising in SPS patients: **BRAF V600E in 18 of 39 (46%)**, **KRAS in 2 of 40 (5.0%)**, and **loss of MLH1/PMS2 expression on IHC in 17 of 45 (38%)**

### Severity Assessment

- Lifetime CRC risk estimated **>50%**, but the specific risk is not well defined — cohorts are small, phenotypically diverse, and subject to referral bias, so this is likely an overestimate
- In a retrospective series of 77 SPS patients: **22 (28.6%) already had CRC at the baseline [[colonoscopy]]**; a further **5 (6.5%) developed CRC on surveillance** (median follow-up 1.3 y), 4 of those 5 arising in serrated polyps <20 mm. **Estimated 5-year CRC risk under surveillance 7%**
- Across two large descriptive series, 31–42% of index cases carried a CRC diagnosis; mean age at diagnosis ~48 years
- CRCs in SPS are predominantly right-sided (5 of 7 CRCs in one small prospective series) and often flat/serrated in morphology
- Synchronous adenomas are found in the majority of SPS patients; conventional adenomas more frequent in SPS patients with CRC

---

## Differential Diagnosis

*No dedicated diagnostic schema covers colorectal polyposis; the workup runs polyp phenotype (number, size, site, histology) at colonoscopy → syndrome criteria → selective germline testing, as under [[#Diagnostics]].*

- **[[familial-adenomatous-polyposis]] / AFAP / [[mutyh-associated-polyposis|MAP]]** — predominantly adenomatous polyps; APC or MUTYH mutations; consider if adenomas co-occur
- **Hereditary mixed polyposis syndrome (HMPS)** — mixed juvenile-adenomatous-hyperplastic polyps; GREM1
- **[[lynch-syndrome]]** — fewer polyps; MMR gene mutations; MSI-H; may co-exist with SPS (MLH1/PMS2 loss found in some SPS CRCs)
- Sporadic serrated/hyperplastic polyps — isolated polyps without meeting SPS criteria
- **[[juvenile-polyposis-syndrome]]** — hamartomatous polyps; SMAD4/BMPR1A

---

## Diagnostics

- **Colonoscopy** with [[polypectomy]] is the diagnostic and surveillance tool — polyp-clearance goals and interval recommendations are detailed under [[#Endoscopic Management]] [[acg-2015-hereditary-gi-cancer]]
- Consider **MUTYH** germline testing when adenomas are concurrently present and/or family history of adenomas exists
- No established role for tumor molecular profiling in guiding management at this time
- No extracolonic cancer surveillance is recommended (Conditional/VLow) — data are insufficient; familial extracolonic cancer risks have not been clearly established

---

## Therapeutics

### Endoscopic Management

- Colonoscopy every 1–3 years with attempted removal of all polyps >5 mm in diameter (Conditional/Low) [[acg-2015-hereditary-gi-cancer]]
- Complete clearance of all polyps ≥1 cm at index colonoscopy is the goal
- Subsequent intervals based on number/size of polyps and co-existing adenomas

### Surgical Management

**Indications for surgery** [[acg-2015-hereditary-gi-cancer]] (Conditional/Low):

- Inability to endoscopically control the growth of serrated polyps
- Development of CRC
- High-grade dysplasia in a serrated polyp that cannot be removed in its entirety

**Colectomy with IRA** is a reasonable option given the risk of metachronous neoplasia; pancolonic distribution makes surgical planning important.

### Family Member Surveillance

**ACG Rec 21** *(Conditional, very low)*: screening recommendations for family members are **currently unclear pending further data** and should be individualized on the results of the FDR's baseline evaluation [[acg-2015-hereditary-gi-cancer]]. Two published schemes are cited in that guideline's evidence summary — neither is an ACG recommendation:

| Scheme | Age to start FDR colonoscopy | Interval |
|---|---|---|
| Oquinena *et al.* | **35 y**, or **10 y before the youngest age of SPS onset in the family** — whichever comes first | Every **3–5 y** if no polyps; **annual** if the FDR meets SPS criteria 1 or 3; **every 2 y** if only criterion 2 |
| NCCN | Earliest of: **age 40**, the youngest SPS diagnosis in the family, or **10 y before the family's CRC in an SPS patient** | Not specified |

- Rationale for screening at all: incidence of SPS among 78 prospectively screened FDRs was 32%, and FDRs of SPS patients had five times the incidence of CRC in one study

---

## See Also

[[familial-adenomatous-polyposis]], [[mutyh-associated-polyposis]], [[lynch-syndrome]], [[juvenile-polyposis-syndrome]], [[colorectal-cancer]], [[colorectal-cancer-screening]], [[colonoscopy]], [[polypectomy]]

---

## Sources

1. [[acg-2015-hereditary-gi-cancer|ACG 2015: Genetic Testing and Management of Hereditary Gastrointestinal Cancer Syndromes]]
