---
title: "Disconnected Pancreatic Duct (DPD) and DPD Syndrome"
...
```mermaid
flowchart TD
    A["Necrotizing pancreatitis<br/>>30% necrosis along the MPD, or pancreatic trauma"] --> B["Early arterial-phase CT<br/>central necrosis + necrotic collection?"]
    B --> C["Wait until the collection is significantly reduced in size<br/>(avoid obscuring/compressing the MPD)"]
    C --> D["MRCP ± secretin — before the final necrosectomy session"]
    D -->|"MRCP clearly delineates DPD"| E["Diagnosis secure — proceed to intervention<br/>(ERCP not required to confirm)"]
    D -->|"Equivocal"| F["ERCP = gold standard<br/>only after pancreatic drainage is established"]
    E --> G["At LAMS removal: leave ≥2 double-pigtail stents in the residual cavity"]
    F --> G
```
