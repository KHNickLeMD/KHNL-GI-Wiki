---
title: "Seronegative Villous Atrophy"
...
```mermaid
flowchart TD
    A["Villous atrophy on biopsy"] --> B["Total IgA + IgA tTG"]
    B --> C["tTG positive"] --> C1["Celiac disease"]
    B --> D["IgA deficient"] --> D1["IgG tTG + IgG DGP<br/>(± IgG EMA)"]
    D1 --> D2["Positive → celiac disease<br/>with selective IgA deficiency"]
    D1 --> E
    B --> F["tTG negative,<br/>total IgA normal"] --> F1["IgA DGP + IgA EMA"]
    F1 --> F2["Positive → celiac disease"]
    F1 --> E["Review histology with pathologist"]
    E --> G["Review medical, travel,<br/>and medication history"]
    G --> H{"Clinical + histologic review<br/>suggest an etiology<br/>other than celiac disease?"}
    H -->|Yes| I["Test for CVID, SIBO,<br/>Giardia, HIV,<br/>autoimmune enteropathy"]
    I --> J{"Etiology identified?"}
    J -->|Yes| K["Treat the cause"]
    J -->|No| L["Unspecified enteropathy<br/>→ treat empirically<br/>with budesonide"]
    H -->|No| M["Obtain HLA"]
    M -->|Negative| I
    M -->|Positive| N["Treat empirically with GFD"]
    N --> O["Reassess symptoms and<br/>repeat biopsy to<br/>assess for improvement"]
```
