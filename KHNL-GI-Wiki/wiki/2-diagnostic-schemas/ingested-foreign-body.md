---
title: "Ingested Foreign Body and Food Bolus Impaction"
...
```mermaid
flowchart TD
    A["Suspected ingestion or food bolus impaction"] --> B{"Airway / ventilatory status<br/>Able to manage secretions?"}
    B -- "No — cannot manage secretions" --> C["EMERGENT endoscopy<br/>± endotracheal intubation"]
    B -- "Yes" --> D{"Peritonitis or<br/>small-bowel obstruction?"}
    D -- "Yes" --> E["Surgical consultation<br/>do not delay with endoscopy"]
    D -- "No" --> F{"Suspected uncomplicated<br/>nonbony food bolus?"}
    F -- "Yes" --> G["Endoscopy without radiographs"]
    F -- "No" --> H["Biplane radiographs<br/>no contrast study"]
    H --> I{"Object seen?"}
    I -- "No, but persistent<br/>esophageal symptoms" --> J["Endoscopy anyway"]
    I -- "Yes" --> K["Triage by object class,<br/>size and location"]
```
