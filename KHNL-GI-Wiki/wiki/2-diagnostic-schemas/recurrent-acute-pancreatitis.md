...
```mermaid
flowchart TD
    A["Recurrent acute pancreatitis<br/>(≥2 episodes, resolution, ≥3 mo apart)"] --> B["Standard workup:<br/>history, meds, labs<br/>(TG, Ca), noninvasive imaging"]
    B -->|Unrevealing| C["EUS (preferred)<br/>2–6 wk after AP resolution"]
    C -->|Unrevealing| D["MRI/MRCP with contrast<br/>(± secretin)"]
    C -->|Occult stones / lesion| E["Directed therapy"]
    D -->|Pancreas divisum| F["See pancreas divisum<br/>± minor papilla endotherapy"]
    D -->|Standard anatomy| G["ERCP controversial —<br/>shared decision-making;<br/>biliary sphincterotomy alone"]
```
