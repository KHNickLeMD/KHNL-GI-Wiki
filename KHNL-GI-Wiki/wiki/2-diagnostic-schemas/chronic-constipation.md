...
```mermaid
flowchart TD
    A["Adult with chronic constipation"] --> B{"Alarm features?<br/>rectal bleeding, heme-positive stool,<br/>iron-deficiency anaemia, weight loss"}
    B -->|Yes| C["Colonoscopy to exclude organic disease"]
    B -->|No| D{"Age at or above the<br/>CRC screening start age<br/>and never screened?"}
    D -->|Yes| C
    D -->|No| E["No endoscopy in the initial evaluation"]
    C --> E
    ...
    L -->|Abnormal| M["Defecatory disorder<br/>→ pelvic floor biofeedback course"]
    L -->|Normal| N["Colonic transit testing off treatment"]
    M --> O{"Still symptomatic after<br/>an adequate trial?"}
    N --> O
    O -->|Yes| P["Refractory constipation:<br/>repeat transit on maximal laxatives,<br/>consider defecography,<br/>trial all OTC + FDA-approved agents"]
    O -->|No| K
    P --> Q{"Slow transit confirmed AND<br/>pelvic floor dysfunction excluded?"}
    Q -->|Yes| R["Surgical referral<br/>see Chronic Idiopathic Constipation"]
