---
title: "Baveno VIII — Advancing Consensus in Portal Hypertension (2026)"
category: source
tags: [portal-hypertension, cirrhosis, cACLD, CSPH, HVPG, liver-stiffness, spleen-stiffness, ANTICIPATE, NICER, NSBB, carvedilol, varices, variceal-bleeding, TIPS, decompensation, recompensation, budd-chiari, portal-vein-thrombosis, psvd, ncpf, anticoagulation, masld, paediatric]
created: 2026-09-03
updated: 2026-09-03
sources: []
---
...
*Three of the source's five figures are recreated below as a table and two flowcharts; see [[#Contradictions / Open Questions]] for the parts of the remaining figures the text does not state.*
...
```mermaid
flowchart TD
    A["ACLD / cirrhosis — suspected upper GI<br/>portal hypertension–related bleeding"] --> B["INITIAL MANAGEMENT (parallel)<br/>ICU or intermediate care<br/>Restrictive pRBC, target Hb 7–8 g/dL<br/>Vasoactive therapy ASAP, 2–5 days or until TIPS<br/>Preventive 3rd-gen cephalosporin (Child-Pugh A: shorten/omit)<br/>Erythromycin 250 mg IV, 30–90 min pre-endoscopy<br/>Intubate if haematemesis or altered consciousness<br/>Lactulose to prevent/treat overt HE<br/>No routine FFP, platelets, rFVIIa, or tranexamic acid"]
    B --> C["Upper endoscopy within 12 h of admission"]
    C --> D{"Source of bleeding"}
    D -->|"Oesophageal varices or GOV1"| E["Endoscopic band ligation"]
    D -->|"GOV2 or IGV1"| F["BIMODAL APPROACH<br/>acute + definitive phase in parallel, or staged with<br/>the shortest possible interval"]
    E --> G{"Bleeding controlled?"}
    G -->|"No"| H["Covered SEMS as bridge<br/>(preferred over balloon tamponade)<br/>→ salvage TIPS + assess LT candidacy"]
    G -->|"Yes"| I{"Meets p-TIPS criteria?<br/>Child-Pugh C10–C13, or<br/>Child-Pugh B >7 + active bleeding, or<br/>HVPG ≥20 mmHg"}
    I -->|"Yes"| J["Pre-emptive TIPS within 72 h<br/>(ideally <24 h)"]
    I -->|"No"| K["Secondary prophylaxis:<br/>NSBB + EBL, carvedilol preferred<br/>Repeat EBL q2–4 weeks until eradication"]
    K -->|"Rebleeding"| L["Consider TIPS"]
    F --> M["ACUTE PHASE — immediate haemostasis<br/>EUS-guided coil + cyanoacrylate preferred where available<br/>(experienced centres); conventional endoscopic<br/>cyanoacrylate injection is equally effective for initial haemostasis"]
    F --> N["DEFINITIVE PHASE — durable control<br/>TIPS ± variceal embolisation, as soon as possible<br/>If TIPS contraindicated: transvenous obliteration,<br/>endoscopic/EUS-guided therapy, or surgical methods<br/>after multidisciplinary discussion"]
```
...
```mermaid
flowchart TD
    A["Recent or chronic PVT<br/>without cirrhosis"] --> B{"Permanent risk factor?"}
    B -->|"Major<br/>(thrombophilia, MPN)"| C["HIGH risk of recurrence<br/>Long-term full-dose anticoagulation"]
    B -->|"Present but not major"| D["INTERMEDIATE / undetermined risk<br/>Consider DOACs at lower doses"]
    B -->|"Absent"| E{"Reversible risk factor?"}
    E -->|"No (unprovoked)"| D
    E -->|"Yes (provoked)"| F["LOW risk of recurrence<br/>Consider stopping anticoagulation"]
```
...
153 priority topics across the nine panels. Themes most relevant to open clinical questions:
...
- **Serum-marker cut-offs confirming an index LSM ≥10 kPa are not restated.** Baveno VII 2.11 named FIB-4 ≥2.67, ELF ≥9.8, FibroTest ≥0.58 (alcohol/viral) or ≥0.48 (NAFLD). Baveno VIII 1.4 requires only "another validated NIT of advanced fibrosis." Those numbers are Baveno VII's, not Baveno VIII's.
...
- **"Long-term DOACs at lower dose" (9.10b) has no dose in the statement.** The only concrete figure in the document is in the research agenda (RA9.3), which asks about "DOACs *other than* **rivaroxaban 15 mg/day**" — implying rivaroxaban 15 mg/day is the studied lower dose, but the statement itself does not prescribe it.
...
- **PSVD scoring bands below 3 points.** The Figure 4 total-score axis prints tick marks at 0–1, 2, 3–4 and 5+, but only the **≥5 (definite)** and **3–4 (possible)** bands are labelled in the text; the name of the ≤2 band is given only in the figure itself.
- **Referenced supplementary material is not reproduced here.** Supplementary Tables S1 (frailty/sarcopenia assessment tools), S2 (conditions excluded from the PSVD/NCPF definition), S3 (conditions associated with PSVD/NCPF), and S4 (permanent vs reversible PVT risk factors) are cited by statements 4.16, 8.40, 8.45, and 9.5 but are published separately. The **EASL CPG on Vascular Liver Diseases**, cited by 8.3 for the VLD aetiological workup, is not summarized on this site.
...
- **Publication status.** This summary is based on the **accepted pre-proof (31 July 2026)**, not the version of record; the publisher notes that copyediting may still alter content. One internal inconsistency already visible: statement 1.16c gives the SSM rule-in as **>55 kPa** while Figures 1 and 2 print **≥55 kPa**. This page uses the statement text.
