...
**Run `git pull --rebase` in the repo root first, every session.** The server's lint cron (twice daily, 06:00 and 18:00 Pacific) pushes to `origin/main` on its own schedule, so any clone that has been idle is behind. Editing a stale copy produces avoidable merge conflicts on pages the cron has since re-linted (2026-08-09: an 8-day-stale laptop clone conflicted on three pages). Pull, then edit.
...
Wiki pages are read by clinicians. **Never write instructions to yourself, or notes about the ingest process, on a page.** Banned on any page other than `log.md` and `needed-sources.md`: "do not reconstruct from memory", "flagged, not filled", "would be needed to add…", "not in any ingested source", "corpus", "corpus-blocked", "decision gap", "source-fidelity flag", tool names (`pdftotext`, PyMuPDF, text layer, raw PDF, `raw/` paths), flag dates, and "the wiki asserts/should…".

When a source omits a criterion, score, dose, or figure, the page states it once, plainly, for the reader — *"ACG 2021 does not quantify 'large'."* / *"Rome V gives the cut-points but not the item weights; assign the stratum from the clinical profile."* — and nothing more. The reasoning, the flag, and the maintenance instruction go in the log entry. The paper or table that would fill the gap goes in **`wiki/needed-sources.md`** (citation · what it fills · page slug) — that page is Nick's download list; every gap that names a specific resource gets a line there, and the line is removed when the resource is ingested.
...
- **Record as you go — scheduled passes especially (Nick, 2026-09-22).** Five cron passes in a row ingested sources and created pages without a `log.md` entry, because the log was left for the end and the run stopped first. So: write this pass's `log.md` entry header before any other edit, and the moment a source page or new entity page is written, add its `index.md` row and a one-line bullet to that entry. Finish the entry at the end, but never leave it for last. A pass that changed any page under `wiki/` without touching `wiki/log.md` is incomplete.
...
- **Fill coverage gaps from the queue — slowly (every pass).** `wiki/index.md` → *Coverage Gaps* holds a prioritized list of topics that have **no page but are substantively covered by already-ingested sources** (Nick, 2026-09-03). Each pass, create **1–2 pages from the top of that queue** — no more, regardless of how many remain; if the pass has already spent heavily on ingest or stub expansion, do **1 or none**. Build each page strictly from the ingested sources named beside it (same no-outside-information rule as stub expansion), route it to the correct schema folder, add it to its index section, link it from the pages that currently mention the topic in plain text, and remove it from the queue. When the queue is empty, add newly noticed gaps to it (with their supporting sources) rather than filling them in the same pass. Gaps with **no** ingested source go on `wiki/needed-sources.md` and are never written from general knowledge.
...
**Guideline-sourced cards only (Nick, 2026-08-11).** A card may only be written from a **tier-1 source** — society guideline, clinical practice update, or consensus statement. **No cards from RCTs, cohorts, meta-analyses, conference abstracts, or lectures/chalk talks** — "not yet," so the rule may be relaxed later, but until then it binds every ingest, every page edit, and the nightly card pass. This is about the deck, not the wiki: RCT and abstract content still reaches wiki pages as normal. When a page's clinical content comes from a mix of tiers, card only the guideline-backed facts and leave the rest uncarded. The card's `{source-slug}` is the test — if it doesn't point at a guideline/CPU/consensus page, the card isn't written.
